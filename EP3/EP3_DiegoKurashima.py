"""
  ...
  A função floor foi importada pelo autor do programa, como permitido pelos docentes.
...
def determina_cores(I, R, ro):
    ...
    """
    # lista com listas dos centros e cores correspondentes, em que em cada lista: 1º é a cor e 2º é o centro(x)
    Centros=[]
    ...
    for i in range(len(R)):
        ...
        for j in range(len(R[0])):
            ...
            if I[i][j] == -1:
                ...
                if Centros==[]:
                    cor=nova_cor()
                    linhaCentros.append(cor)
                    linhaCentros.append(R[i][j])
                    linhaC.append(cor)
                    ...
                    for centro in Centros:
                         ...
                         # se é associado a um centro
                         if abs((R[i][j])-(centro[1])) <= ro and Associado==False:
                                linhaC.append(centro[0])
                                Associado=True

                    #é centro novo, pois não foi associado
                    if Associado==False:
                        cor=nova_cor()
                        linhaCentros.append(cor)
                        linhaCentros.append(R[i][j])
...
def calcula_sombras(I):
    ...
    #analisando as entradas e k em I, atribuindo à lista a
    for i in range(len(I)):
...
def grava_imagem(I, C, s, alfa, nome_arquivo):
    ...
    for i in range(len(C)):
        ...
        for j in range(len(C[i])):
            ...
            arquivo.write(str(floor((1-alfa+alfa*s[I[i][j]])*C[i][j][0]))+' '+str(floor((1-alfa+alfa*s[I[i][j]])*C[i][j][1]))+' '+str(floor((1-alfa+alfa*s[I[i][j]])*C[i][j][2]))+' ')
